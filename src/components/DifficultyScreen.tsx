  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center px-3 py-2 border-b border-line bg-panel/70">
        ...
        <div className="ml-3 text-sm">Chọn độ khó</div>
      </div>
        ...
        {items.map((d) => {
          ...
          return (
            <button
              key={d}
              onClick={() => onPick(d)}
              className="w-full text-left rounded-2xl border border-line bg-panel p-4 active:scale-[.99]"
            >
              <div className="text-base font-medium">{DIFFICULTY_LABEL[d]}</div>
              <div className="text-xs text-soft mt-1">{DIFFICULTY_DESC[d]}</div>
              <div className="text-[11px] text-soft mt-2 tracking-wider">
                ¥{s.money} · ⚡{s.energy} · 🔥{s.stress} · あ{s.language} · ♡
                {s.relationship}
              </div>
            </button>
          );
